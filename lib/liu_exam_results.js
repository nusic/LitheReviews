...
var LIU_EXAM_RESULTS_URL = 'http://www4.student.liu.se';

function getUri(course){ 
    return '/tentaresult/?kurskod=' + course.code 
...
function extractStats(htmlStr, course){
  ...
  function examToArrays(exam){
    return $.map(exam, function (stats, examType){
      ...
      var statsArray = [], gradeOrdering = ['U', 'G', '3', '4', '5'];
      gradeOrdering.forEach(function(grade){
        if(stats[grade]) statsArray.push({grade: grade, freq: stats[grade]});
      });

      console.log(examType, statsArray);
      return [{examType: examType, stats: statsArray}];
    });
  };
...
exports.search = function(course, callback) {
  ...
  var url = LIU_EXAM_RESULTS_URL + getUri(course);
